...
for m in range(1, loop+1):
    ...
    for i in range(1, num_lambda + 1):
        ...
        for epoch in range(num_epoch):
            ...
            boundary_point = torch.tensor([[0.0], [np.pi]], requires_grad=True, device=device)
            ...
            du_boundary_dx = torch.autograd.grad(outputs=boundary_predict, inputs=boundary_point,
                                                 grad_outputs=torch.ones_like(boundary_predict), retain_graph=True,
                                                 create_graph=True)[0]
            loss_boundary = (du_boundary_dx[0]) ** 2 + (du_boundary_dx[1]) ** 2
